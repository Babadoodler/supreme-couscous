<script lang="ts">
  ...
  // Style URLs are deliberately single constants — swappable tiles (DESIGN.md §12.1).
  ...
  let darkStyleFailed = false;

  function preferredStyle(): string {
    return darkQuery?.matches && !darkStyleFailed ? DARK_STYLE : LIGHT_STYLE;
  }
  ...
  onMount(() => {
    ...
    // Custom sources/layers must be re-added after every setStyle (theme swap).
    map.on('style.load', ensureLayers);
    map.on('load', () => {
      // Auto-fit once on open; never re-fit while the user is interacting (§6.2).
      fitToStops(false);
    });
    // If the dark style can't be fetched, fall back to the light one once.
    map.on('error', () => {
      if (!styleReady && !darkStyleFailed && darkQuery?.matches) {
        darkStyleFailed = true;
